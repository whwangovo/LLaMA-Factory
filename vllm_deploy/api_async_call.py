...
# 创建异步客户端
client = AsyncOpenAI(api_key="0", base_url="http://0.0.0.0:8000/v1")
...
async def get_three_responses(prompt: str) -> Dict[str, str]:
    """异步获取三个响应"""
    tasks = [get_response(prompt) for _ in range(3)]
...
async def process_all_prompts(prompts: List[str]) -> List[Dict]:
    """处理所有提示词"""
    # 创建所有提示词的任务
    tasks = [get_three_responses(prompt) for prompt in prompts]
    ...
    for task in asyncio.as_completed(tasks):
        ...
        # 每次有新结果就保存到文件
        with open("finetune_output_250125_async.json", "w", encoding="utf8") as f:
...
async def main():
    ...
    results = await process_all_prompts(prompts)
